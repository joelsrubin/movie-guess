...
export function YearFilter({ availableYears, selectedYears, onYearsChange, error, setErrors }: YearFilterProps) {
  ...
  const isDecadeFullySelected = (decade: number) => {
    ...
    return decadeYears.length > 0 && decadeYears.every((year) => selectedYears.includes(year))
  }

  const isDecadePartiallySelected = (decade: number) => {
    const decadeYears = getYearsInDecade(decade)
    return decadeYears.some((year) => selectedYears.includes(year)) && !isDecadeFullySelected(decade)
  }
          ...
          {decades.map((decade) => {
            ...
            return (
              <div key={decade} className="flex items-center gap-2">
                ...
                >
                  {decade}s {isPartiallySelected && !isFullySelected ? "•" : ""}
                </Badge>
